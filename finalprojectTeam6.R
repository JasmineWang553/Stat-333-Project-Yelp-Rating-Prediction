...
write.csv(combine, "ytableworddf.csv")
write.csv(combineout, "yelp_outwordddf.csv")

ytworddf = read.csv("ytableworddf.csv", header =T) 
ytworddf = ytworddf[,-1]
#ytworddf = ytworddf[-1,]
ytworddf =ytworddf[,-117]

####yelpout
yoworddf = read.csv("yelp_outwordddf.csv", header = T)
yoworddf = yoworddf[,-1]
#yoworddf = yoworddf[-1,]
yoworddf =yoworddf[,-117]

##binding word and tables
...
ytable = cbind(ytable, ytworddf)
yelp_out = cbind(yelp_out, yoworddf)
...
countrydf = splitfunction(pnsvector, country)
countrydfout = splitfunction(pnsvectorout, country)
...
###remove date
yt = yt[,-c(which(colnames(yt) == "Year"), which(colnames(yt) == "date"))]
yo = yo[,-c(which(colnames(yo) == "Year"), which(colnames(yo) == "date"))]
...
write.csv(phrasedfout, "phrasedfout.csv")
